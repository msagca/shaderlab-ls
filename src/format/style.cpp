...
bool run(const std::vector<std::string>& argv, std::string_view input, std::string& output) {
  ...
  if (!CreatePipe(&inRead, &inWrite, &inherit, 0)) return false;
  ...
  BOOL started = CreateProcessW(nullptr, line.data(), nullptr, nullptr, TRUE, CREATE_NO_WINDOW, nullptr, nullptr,
                                &startup, &process);
  CloseHandle(inRead);
  ...
  std::thread writer([inWrite, input] {
    for (size_t at = 0; at < input.size();) {
      ...
      DWORD written = 0;
      if (!WriteFile(inWrite, input.data() + at, chunk, &written, nullptr) || written == 0) break;
      at += written;
    }
    CloseHandle(inWrite);
  });
...
bool run(const std::vector<std::string>& argv, std::string_view input, std::string& output) {
  ...
  for (const std::string& argument : argv) arguments.push_back(const_cast<char*>(argument.c_str()));
...
bool hidden(std::string_view line) {
  ...
  if (text.size() >= 2 && text.front() == '[' && text.back() == ']') return true;
...
std::string hide(std::string_view code, std::vector<std::string>& lines) {
  ...
  for (std::string_view line : splitLines(code)) {
    ...
    if (result.empty() && text.empty()) continue;
...
std::string unhide(std::string_view formatted, const std::vector<std::string>& lines) {
  ...
  if (!result.empty()) result.pop_back();  // splitLines() already gave the trailing newline a line of its own
  return result;
}
...
bool hasConfigFile(const std::filesystem::path& start) {
  ...
  for (std::filesystem::path dir = start; !dir.empty();) {
    if (std::filesystem::exists(dir / ".clang-format", error)) return true;
    if (std::filesystem::exists(dir / "_clang-format", error)) return true;
...
std::map<std::string, std::string> parseConfig(std::string_view config) {
  ...
  for (std::string_view line : splitLines(config)) {
    ...
    if (colon == std::string_view::npos || text.empty() || text.front() == '-' || text.front() == '#') continue;
    std::string_view value = trim(text.substr(colon + 1));
    if (!value.empty()) values.emplace(text.substr(0, colon), value);
  }
  return values;
}
...
std::vector<std::string> ClangFormat::arguments(std::string_view mode) const {
  ...
  if (!assumeFilename_.empty()) arguments.push_back("--assume-filename=" + assumeFilename_);
...
std::optional<std::string> ClangFormat::format(std::string_view code) const {
  ...
  std::vector<std::string> lines;
  std::string output;
  if (!run(arguments("-"), hide(code, lines), output)) return std::nullopt;
...
std::optional<std::string> ClangFormat::dumpConfig() const {
  if (!available()) return std::nullopt;
  std::string output;
  if (!run(arguments("--dump-config"), {}, output)) return std::nullopt;
  return output;
}
...
FormatOptions resolveStyle(const std::filesystem::path& file, const std::string& clangFormatExe) {
  ...
  if (useTab != values.end()) options.useTabs = useTab->second != "Never";
  auto afterStruct = values.find("AfterStruct");
  if (afterStruct != values.end()) options.bracesOnOwnLine = afterStruct->second == "true";
