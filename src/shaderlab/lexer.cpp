...
const char *blockKeyword(BlockKind kind) {
  switch (kind) {
  case BlockKind::HlslProgram:
    return "HLSLPROGRAM";
  case BlockKind::HlslInclude:
    return "HLSLINCLUDE";
  case BlockKind::CgProgram:
    return "CGPROGRAM";
  case BlockKind::CgInclude:
    return "CGINCLUDE";
  case BlockKind::GlslProgram:
    return "GLSLPROGRAM";
  }
  return "";
}
  ...
  while (i < src.size()) {
    ...
    if (c == '/' && i + 1 < src.size() && src[i + 1] == '/') {
      ...
      while (i < src.size() && src[i] != '\n')
        ++i;
      size_t end = i;
      if (end > begin && src[end - 1] == '\r')
        --end;
      if (keepComments)
        push(Tok::Comment, begin, end);
      continue;
    }
    if (c == '/' && i + 1 < src.size() && src[i + 1] == '*') {
      size_t begin = i;
      ...
      if (keepComments)
        push(Tok::Comment, begin, i)->terminated = close != std::string_view::npos;
      continue;
    }
    size_t begin = i;
    if (c == '"') {
    ...
    ++i;
    if (kind == Tok::Unknown) {
      while (i < src.size() && (static_cast<unsigned char>(src[i]) & 0xC0) == 0x80)
        ++i;
    }
    push(kind, begin, i);
  }
...
std::string stringValue(const Token &token) {
  ...
  if (token.stringTerminated && !text.empty() && text.back() == '"')
