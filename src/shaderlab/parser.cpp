...
class Parser {
 public:
  explicit Parser(std::string_view source) : src_(source), toks_(lexShaderLab(source)), lines_(source) {}
  ...
  ShaderFile run() {
    ...
    while (!atEnd()) {
      ...
      if (isWord(token, "Shader")) {
        if (out_.hasShader) report(token.span, "Only one Shader block is allowed in a file.");
        parseShader();
        continue;
      }
      if (lineOf(token) != lastJunkLine) {
        report(token.span, token.kind == Tok::CodeBlock ? "Shader code blocks must be inside a Shader block."
 ...
 private:
  ...
  size_t lineOf(const Token& token) const { return lines_.lineOf(token.span.begin); }
  ...
  void skipStatement(const Token& start) {
    ...
    while (!atEnd()) {
      const Token& token = peek();
      if (advanced && lineOf(token) != line) break;
      if (token.kind == Tok::RBrace) break;
      if (advanced && token.kind == Tok::CodeBlock) break;
  ...
  void skipBalanced() {
    ...
    while (!atEnd()) {
      ...
      if (token.kind == Tok::RBrace && --depth <= 0) return;
    }
  }
  ...
  std::optional<std::string> expectString(const Token& anchor, std::string_view what) {
    ...
    if (token.kind == Tok::String) {
      advance();
      if (!token.stringTerminated) report(token.span, "Unterminated string.");
  ...
  void recordBlock(const Token& token, int subShader, int pass) {
    ...
    block.pass = pass;
    if (!token.terminated) {
      const char* terminator = isCgBlock(token.block) ? "ENDCG" : token.block == BlockKind::GlslProgram ? "ENDGLSL" : "ENDHLSL";
  ...
  void parseShader() {
    ...
    if (peek().kind == Tok::String) {
      ...
      if (!name.stringTerminated) report(name.span, "Unterminated string.");
    ...
    size_t scope = 0;
    if (!openBrace(keyword, ScopeKind::Shader, ScopeKind::Shader, scope)) return;
  ...
  void parseProperties() {
    const Token& keyword = advance();
    if (!out_.propertiesKeyword.empty()) report(keyword.span, "Duplicate Properties block.", Severity::Warning);
    out_.propertiesKeyword = keyword.span;
    size_t scope = 0;
    if (!openBrace(keyword, ScopeKind::Properties, ScopeKind::Shader, scope)) return;
    while (!atEnd() && peek().kind != Tok::RBrace) {
      size_t before = pos_;
      parseProperty();
  ...
  void parseProperty() {
    ...
    while (peek().kind == Tok::LBracket) {
      ...
      if (name.empty()) report(attribute.span, "Empty property attribute.");
      property.attributes.push_back(std::move(attribute));
      while (!atEnd() && peek().span.begin <= close) advance();
    }
    ...
    if (peek().kind != Tok::LParen) return fail(peek(), "Expected '(' after property name '" + property.name + "'.");
    advance();
    if (peek().kind != Tok::String) return fail(peek(), "Expected a display name string.");
    property.displayName = stringValue(advance());
    if (peek().kind != Tok::Comma) return fail(peek(), "Expected ',' after the display name.");
    advance();

    const Token& typeToken = peek();
    if (typeToken.kind != Tok::Ident) return fail(typeToken, "Expected a property type.");
    advance();
    ...
    if (!typeEntry) {
      report(typeToken.span, "Unknown property type '" + property.type + "'. Expected one of: " +
                                 joinNames(ref::propertyTypes()) + ".");
    }
    if (iequals(property.type, "Range")) {
      ...
      if (!ok) return fail(peek(), "Expected 'Range(min, max)'.");
    ...
    if (peek().kind != Tok::RParen) return fail(peek(), "Expected ')' after the property type.");
    advance();
    if (peek().kind != Tok::Equals) return fail(peek(), "Expected '=' and a default value.");
    advance();

    enum class Default { None, Number, Tuple, Texture } defaultKind = Default::None;
    ...
    if (valueToken.kind == Tok::Number) {
      ...
      advance();
      while (!atEnd() && (peek().kind == Tok::Number || peek().kind == Tok::Comma)) advance();
      if (peek().kind != Tok::RParen) return fail(peek(), "Expected ')' to close the default value.");
      advance();
      defaultKind = Default::Tuple;
    } else if (valueToken.kind == Tok::String) {
      advance();
      defaultKind = Default::Texture;
      if (peek().kind == Tok::LBrace) skipBalanced();
    } else {
    ...
    if (typeEntry) {
      ...
      Default expected = numeric ? Default::Number : vector ? Default::Tuple : Default::Texture;
  ...
  void parseSubShader() {
    ...
    size_t scope = 0;
    if (!openBrace(keyword, ScopeKind::SubShader, ScopeKind::Shader, scope)) return;
    bool first = true;
    while (!atEnd() && peek().kind != Tok::RBrace) {
      const Token& token = peek();
      if (token.kind == Tok::CodeBlock) {
        ...
        if (!first) report(token.span, "A PackageRequirements block must come before all other declarations inside the SubShader.");
  ...
  void parsePass(int subShader) {
    ...
    size_t scope = 0;
    if (!openBrace(keyword, ScopeKind::Pass, ScopeKind::SubShader, scope)) return;
    bool first = true;
    ...
    while (!atEnd() && peek().kind != Tok::RBrace) {
      ...
      if (token.kind == Tok::CodeBlock) {
        ...
        if (isProgramBlock(token.block)) {
          if (haveProgram) report(out_.blocks.back().keyword, "A Pass can contain only one shader program block.", Severity::Warning);
          haveProgram = true;
        }
        advance();
      } else if (isWord(token, "PackageRequirements")) {
        if (!first) report(token.span, "A PackageRequirements block must come before all other declarations inside the Pass.");
  ...
  void skipLegacyCommand() {
    ...
    while (!atEnd() && lineOf(peek()) == line) {
      const Token& token = peek();
      if (token.kind == Tok::RBrace || token.kind == Tok::LBrace || token.kind == Tok::CodeBlock) break;
  ...
  void parsePackageRequirements(ScopeKind parent) {
    const Token& keyword = advance();
    size_t scope = 0;
    if (!openBrace(keyword, ScopeKind::PackageRequirements, parent, scope)) return;
    while (!atEnd() && peek().kind != Tok::RBrace) {
      const Token& token = peek();
      if (token.kind != Tok::String) {
  ...
  void parseGrabPass() {
    const Token& keyword = advance();
    size_t scope = 0;
    if (!openBrace(keyword, ScopeKind::GrabPass, ScopeKind::SubShader, scope)) return;
    if (peek().kind == Tok::String) advance();
    while (!atEnd() && peek().kind != Tok::RBrace) {
  ...
  void parseTags(int subShader, int pass) {
    ...
    size_t scope = 0;
    bool isPass = pass >= 0;
    if (!openBrace(keyword, ScopeKind::Tags, isPass ? ScopeKind::Pass : ScopeKind::SubShader, scope)) return;
    std::vector<Tag> tags;
    while (!atEnd() && peek().kind != Tok::RBrace) {
  ...
  void validateTag(const Tag& tag) {
    if (iequals(tag.key, "Queue")) {
      ...
      size_t nameEnd = 0;
      while (nameEnd < value.size() && std::isalpha(static_cast<unsigned char>(value[nameEnd]))) ++nameEnd;
      ...
      if (!numeric && (!ref::find(ref::tagValues("Queue"), name) || !offsetOk)) {
        report(tag.valueSpan, "Invalid Queue value '" + tag.value + "'. Expected '[queue name]' or '[queue name] + [offset]' where the queue name is one of: " +
                                  joinNames(ref::tagValues("Queue")) + ".", Severity::Warning);
      }
      return;
    }
    for (std::string_view closed : {"ForceNoShadowCasting", "IgnoreProjector", "PreviewType"}) {
      if (iequals(tag.key, closed) && !ref::find(ref::tagValues(closed), tag.value)) {
        report(tag.valueSpan, "Invalid " + std::string(closed) + " value '" + tag.value + "'. Expected one of: " +
                                  joinNames(ref::tagValues(closed)) + ".", Severity::Warning);
      }
    }
  }
    ...
    while (!atEnd() && lineOf(peek()) == line) {
      ...
      if (token.kind == Tok::Ident) {
        ...
        if (peek().kind == Tok::Ident && peek(1).kind == Tok::RBracket) {
          ...
          while (!atEnd() && lineOf(peek()) == line && peek().kind != Tok::RBracket && peek().kind != Tok::RBrace) advance();
          if (peek().kind == Tok::RBracket) advance();
        }
      } else {
        break;
      }
  ...
  void parseStencil(const Token& keyword, Command& command, ScopeKind owner) {
    size_t scope = 0;
    if (!openBrace(keyword, ScopeKind::Stencil, owner, scope)) return;
    while (!atEnd() && peek().kind != Tok::RBrace) {
      const Token& token = peek();
      if (token.kind != Tok::Ident) {
  ...
  static std::vector<const CommandArg*> values(const Command& command) {
    ...
    for (const CommandArg& arg : command.args) {
      if (!arg.comma) result.push_back(&arg);
    }
    return result;
  }
  ...
  void checkValue(const Command& command, const CommandArg& arg, std::span<const ref::Entry> table, std::string_view what) {
    if (arg.propertyRef || ref::find(table, arg.text)) return;
    report(arg.span, "Invalid " + std::string(what) + " '" + arg.text + "' for " + command.name + ". Expected one of: " +
                         joinNames(table) + ".");
  }

  void checkInteger(const Command& command, const CommandArg& arg, long min, long max, std::string_view what) {
    if (arg.propertyRef) return;
    auto value = parseInteger(arg.text);
    if (!value || *value < min || *value > max) {
      report(arg.span, "Invalid " + std::string(what) + " '" + arg.text + "' for " + command.name + ". Expected an integer from " +
                           std::to_string(min) + " through " + std::to_string(max) + ".");
    }
  }

  bool requireArgCount(const Command& command, size_t count) {
    auto vals = values(command);
    if (vals.size() == count && commaCount(command) == 0) return true;
    if (vals.empty()) {
  ...
  void validateCommand(const Command& command) {
    ...
    if (!single.empty() && name != "BlendOp") {
      if (!requireArgCount(command, 1)) return;
      ...
      if (!accepted) checkValue(command, arg, single, "value");
      return;
    }
    if (name == "BlendOp") {
      ...
      for (const CommandArg* arg : vals) checkValue(command, *arg, ref::blendOperations(), "blending operation");
      return;
    }
    if (name == "Blend") {
      size_t i = 0;
      ...
      if (!factors) {
        ...
        report(span, "Invalid Blend syntax. Expected 'Blend Off', 'Blend <source factor> <destination factor>', or "
      ...
      for (const auto& g : groups) {
        for (const CommandArg* arg : g) checkValue(command, *arg, ref::blendFactors(), "blend factor");
      }
      return;
    }
    if (name == "ColorMask") {
      ...
      if (!channels.propertyRef && channels.text != "0") {
        ...
        for (char c : channels.text) {
          ...
          if (std::string_view("RGBA").find(upper) == std::string_view::npos || seen.find(upper) != std::string::npos) ok = false;
      ...
      if (vals.size() == 2) checkInteger(command, *vals[1], 0, 7, "render target");
      return;
    }
    if (name == "Offset") {
      ...
      for (const CommandArg* arg : values(command)) {
        if (arg->propertyRef) continue;
        char* end = nullptr;
        std::strtod(arg->text.c_str(), &end);
        if (end != arg->text.c_str() + arg->text.size()) report(arg->span, "Expected a float value for Offset.");
      }
      return;
    }
  }
  ...
  void validateStencilField(const Command& field) {
    ...
    if (!requireArgCount(field, 1)) return;
  ...
  void validatePropertyReferences() {
    ...
    auto checkAll = [&](const std::vector<Command>& commands) {
      for (const Command& command : commands) {
        check(command);
        for (const Command& child : command.children) check(child);
      }
    };
    for (const SubShader& subShader : out_.subShaders) {
      checkAll(subShader.commands);
      for (const Pass& pass : subShader.passes) checkAll(pass.commands);
    }
  }
...
ShaderFile parseShaderLab(std::string_view source) { return Parser(source).run(); }
