...
constexpr Entry kShaderBlock[] = {
    {"Properties", "Properties { <Material property declaration> ... }",
     ...
     "Properties\n{\n\t$0\n}"},
    {"SubShader", "SubShader { <optional: LOD> <optional: tags> <optional: commands> <One or more Pass definitions> }",
     "Defines a SubShader. You can define as many Passes as you like within a SubShader.", "SubShader\n{\n\t$0\n}"},
...
constexpr Entry kSubShaderBlock[] = {
    ...
    {"Tags", "Tags { \"[name1]\" = \"[value1]\" \"[name2]\" = \"[value2]\" }",
     "Applies the given tags to the SubShader. You can define as many tags as you like.",
     "Tags { \"$1\" = \"$2\" }"},
    {"Pass", "Pass { <optional: name> <optional: tags> <optional: commands> <optional: shader code> }",
     "Defines a Pass.", "Pass\n{\n\t$0\n}"},
...
constexpr Entry kPassBlock[] = {
    ...
    {"Tags", "Tags { \"<name1>\" = \"<value1>\" \"<name2>\" = \"<value2>\" }",
     "Applies the given tags to the Pass. You can define as many tags as you like.", "Tags { \"$1\" = \"$2\" }"},
    {"HLSLPROGRAM", "HLSLPROGRAM [source code for shader programs] ENDHLSL",
     "Creates a shader program block. Unity adds the HLSL shader program to the pass that includes this shader program "
     "block.",
     "HLSLPROGRAM\n$0\nENDHLSL"},
...
constexpr Entry kCommands[] = {
    ...
    {"Conservative", "Conservative <enabled>",
...
constexpr Entry kPropertyTypes[] = {
    {"Integer", "_ExampleName (\"Integer display name\", Integer) = 1",
     "Backed by a real integer (unlike the legacy `Int` type, which is backed by a float)."},
    {"Int", "_ExampleName (\"Int display name\", Int) = 1",
     "Legacy type backed by a float. Supported for backwards compatibility only; use `Integer` instead."},
    {"Float", "_ExampleName (\"Float display name\", Float) = 0.5", "A float value."},
    {"Range", "_ExampleName (\"Float with range\", Range(0.0, 1.0)) = 0.5",
     "A float with a slider. The minimum and maximum values for the range slider are inclusive.", "Range(${1:0.0}, ${2:1.0})"},
    ...
    {"Any", "_ExampleName (\"Texture\", Any) = \"\" {}",
     "A texture of any dimension. Not in the current reference; used by Unity's built-in blit shaders."},
    {"Color", "_ExampleName(\"Example color\", Color) = (.25, .5, .5, 1)",
     "Maps to a float4 in your shader code. The Material Inspector displays a color picker."},
...
constexpr Entry kPropertyAttributes[] = {
    ...
    {"PerRendererData", "[PerRendererData]",
     "Indicates that a texture property will be coming from per-renderer data in the form of a MaterialPropertyBlock."},
...
constexpr Entry kPragmas[] = {
    ...
    {"raytracing", "#pragma raytracing <name>",
     "Declares a ray tracing shader program (used by HDRP ray tracing passes). Not in the current pragma reference."},
    {"prefer_hlslcc", "#pragma prefer_hlslcc <graphics APIs>", "Used by Unity's own shaders. Not in the current pragma reference."},
    {"extended_structured_buffer_bindings", "#pragma extended_structured_buffer_bindings",
     "Used by Unity's own shaders. Not in the current pragma reference."},
...
constexpr Entry kDirectives[] = {
    ...
    {"include_with_pragmas", "#include_with_pragmas \"path-to-include-file\"",
     "Unity: include a file and apply the `#pragma` directives it contains."},
...
std::span<const Entry> shaderBlockKeywords() { return kShaderBlock; }
std::span<const Entry> subShaderBlockKeywords() { return kSubShaderBlock; }
std::span<const Entry> passBlockKeywords() { return kPassBlock; }
std::span<const Entry> commands() { return kCommands; }
std::span<const Entry> stencilFields() { return kStencilFields; }
std::span<const Entry> blendFactors() { return kBlendFactors; }
std::span<const Entry> blendOperations() { return kBlendOps; }
std::span<const Entry> cullModes() { return kCull; }
std::span<const Entry> zTestOperations() { return kZTest; }
std::span<const Entry> stencilComparisons() { return kStencilCompare; }
std::span<const Entry> stencilOperations() { return kStencilOps; }
std::span<const Entry> onOff() { return kOnOff; }
std::span<const Entry> trueFalse() { return kTrueFalse; }
std::span<const Entry> colorMaskValues() { return kColorMask; }
std::span<const Entry> propertyTypes() { return kPropertyTypes; }
std::span<const Entry> propertyAttributes() { return kPropertyAttributes; }
std::span<const Entry> subShaderTags() { return kSubShaderTags; }
std::span<const Entry> passTags() { return kPassTags; }
std::span<const Entry> pragmas() { return kPragmas; }
std::span<const Entry> pragmaTargets() { return kTargets; }
std::span<const Entry> pragmaRequires() { return kRequires; }
std::span<const Entry> renderers() { return kRenderers; }
std::span<const Entry> preprocessorDirectives() { return kDirectives; }

std::span<const Entry> tagValues(std::string_view key) {
  if (iequals(key, "RenderPipeline")) return kRenderPipelineValues;
  if (iequals(key, "Queue")) return kQueueValues;
  if (iequals(key, "ForceNoShadowCasting") || iequals(key, "IgnoreProjector")) return kShadowValues;
  if (iequals(key, "PreviewType")) return kPreviewTypeValues;
  if (iequals(key, "RenderType")) return kRenderTypeValues;
  if (iequals(key, "LightMode")) return kLightModeValues;
  if (iequals(key, "PassFlags")) return kPassFlagsValues;
  if (iequals(key, "RequireOptions")) return kRequireOptionsValues;
  if (iequals(key, "UniversalMaterialType")) return kUniversalMaterialTypeValues;
  return {};
}
...
const Entry* findCommand(std::string_view name) { return find(kCommands, name); }
...
const Entry* findKeywordAnywhere(std::string_view name) {
  for (auto table : {shaderBlockKeywords(), subShaderBlockKeywords(), passBlockKeywords(), commands()}) {
    if (const Entry* entry = find(table, name)) return entry;
  }
  return nullptr;
}

std::span<const Entry> commandValues(std::string_view command) {
  if (iequals(command, "AlphaToMask") || iequals(command, "ZWrite")) return kOnOff;
  if (iequals(command, "Conservative") || iequals(command, "ZClip")) return kTrueFalse;
  if (iequals(command, "Cull")) return kCull;
  if (iequals(command, "ZTest")) return kZTest;
  if (iequals(command, "BlendOp")) return kBlendOps;
  return {};
}

std::span<const Entry> stencilFieldValues(std::string_view field) {
  if (istartsWith(field, "Comp")) return kStencilCompare;
  if (istartsWith(field, "Pass") || istartsWith(field, "Fail") || istartsWith(field, "ZFail")) return kStencilOps;
  return {};
}

bool isScopeKeyword(std::string_view word) {
  for (std::string_view keyword : {"Pass", "Tags", "Name", "LOD", "UsePass", "GrabPass", "PackageRequirements",
                                   "SubShader", "Properties", "Fallback", "CustomEditor",
                                   "CustomEditorForRenderPipeline", "Dependency"}) {
    if (iequals(word, keyword)) return true;
  }
  return false;
}

bool isLegacyCommand(std::string_view word) {
  for (std::string_view command : {"AlphaTest", "BindChannels", "Color", "ColorMaterial", "Fog", "Lighting", "Material",
                                   "SeparateSpecular", "SetTexture"}) {
    if (iequals(word, command)) return true;
  }
  return false;
}
