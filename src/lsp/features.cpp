...
json markdown(std::string value) { return {{"kind", "markdown"}, {"value", std::move(value)}}; }
...
std::string spanText(const Analysis& a, Span span) { return a.text.substr(span.begin, span.end - span.begin); }
...
bool inside(Span span, size_t offset) { return offset >= span.begin && offset <= span.end && !span.empty(); }
...
std::vector<std::shared_ptr<const CachedFile>> includedFiles(const Analysis& a, const std::vector<size_t>& units,
  ...
  for (size_t unit : units) {
    for (const HlslInclude& include : a.units[unit].scan.includes) queue.emplace_back(include.path, a.path.parent_path());
  }
  ...
  while (!queue.empty() && files.size() < 400) {
    ...
    if (!resolved || !visited.insert(pathKey(*resolved)).second) continue;
    auto file = loadCachedFile(*resolved);
    if (!file) continue;
    for (const HlslInclude& include : file->scan.includes) queue.emplace_back(include.path, resolved->parent_path());
...
int declPriority(DeclKind kind) {
  switch (kind) {
    case DeclKind::Function: return 0;
    case DeclKind::Struct: return 1;
    case DeclKind::CBuffer: return 2;
    case DeclKind::Variable: return 3;
    case DeclKind::Macro: return 4;
    case DeclKind::Field: return 5;
  }
  return 6;
}
...
const HlslDecl* bestDecl(const std::vector<const HlslDecl*>& candidates) {
  ...
  for (const HlslDecl* decl : candidates) {
    if (!best || declPriority(decl->kind) < declPriority(best->kind)) best = decl;
  }
  return best;
}
...
const HlslDecl* findLocalDecl(const Analysis& a, const std::vector<size_t>& units, std::string_view name) {
  ...
  for (size_t unit : units) {
    for (const HlslDecl& decl : a.units[unit].scan.decls) {
      if (decl.name == name) candidates.push_back(&decl);
    }
  }
...
std::optional<ExternalDecl> findExternalDecl(const std::vector<std::shared_ptr<const CachedFile>>& files, std::string_view name) {
  ...
  for (const auto& file : files) {
    for (const HlslDecl& decl : file->scan.decls) {
      if (decl.name != name || decl.kind == DeclKind::Field) continue;
      if (!best || declPriority(decl.kind) < declPriority(best->decl->kind)) best = ExternalDecl{file, &decl};
    }
  }
  return best;
}
...
int itemKindFor(DeclKind kind) {
  switch (kind) {
    case DeclKind::Function: return kItemFunction;
    case DeclKind::Struct: return kItemStruct;
    case DeclKind::CBuffer: return kItemModule;
    case DeclKind::Variable: return kItemVariable;
    case DeclKind::Field: return kItemField;
    case DeclKind::Macro: return kItemConstant;
  }
...
class Items {
 public:
  explicit Items(const FeatureContext& context) : context_(context) {}

  void add(std::string label, int kind, std::string detail = {}, std::string documentation = {},
           std::string insertText = {}, bool snippet = false) {
    if (!labels_.insert(label).second) return;
    json item{{"label", label}, {"kind", kind}};
    if (!detail.empty()) item["detail"] = std::move(detail);
    if (!documentation.empty()) item["documentation"] = markdown(std::move(documentation));
    if (!insertText.empty()) {
      if (snippet && !context_.snippets) return addPlain(std::move(item), insertText);
  ...
  void addEntries(std::span<const ref::Entry> table, int kind, bool useSnippets = true) {
    for (const ref::Entry& entry : table) {
      ...
      add(std::string(entry.name), kind, std::string(entry.detail), entryMarkdown(entry),
          snippet ? std::string(entry.snippet) : std::string(), snippet);
    }
  }
  ...
  json result() { return {{"isIncomplete", false}, {"items", std::move(items_)}}; }
    ...
    for (size_t i = 0; i < snippetText.size(); ++i) {
      if (snippetText[i] == '$' && i + 1 < snippetText.size()) {
        if (snippetText[i + 1] == '{') {
          ...
          if (colon != std::string::npos && close != std::string::npos && colon < close) plain += snippetText.substr(colon + 1, close - colon - 1);
...
void addDecl(Items& items, const HlslDecl& decl, std::string_view where = {}) {
  if (decl.kind == DeclKind::Field) return;
...
void addFunctions(Items& items, const Analysis& a, const std::vector<size_t>& units, const FeatureContext& context) {
  for (size_t unit : units) {
    for (const HlslDecl& decl : a.units[unit].scan.decls) {
      if (decl.kind == DeclKind::Function) addDecl(items, decl);
    }
  }
  for (const auto& file : includedFiles(a, units, context)) {
    for (const HlslDecl& decl : file->scan.decls) {
      if (decl.kind == DeclKind::Function) addDecl(items, decl, file->path.filename().string());
    }
  }
}
...
json hlslCompletion(const Analysis& a, size_t unit, size_t offset, const FeatureContext& context) {
  ...
  if (!trimmed.empty() && trimmed.front() == '#') {
    ...
    size_t wordEnd = 0;
    while (wordEnd < after.size() && isIdentChar(after[wordEnd])) ++wordEnd;
    if (wordEnd == after.size()) {
      items.addValues(ref::preprocessorDirectives(), kItemKeyword);
      return items.result();
    }
    if (after.substr(0, wordEnd) != "pragma") return items.result();
    ...
    if (rest.empty()) return items.result();
    if (complete == 0) {
  ...
  for (size_t u : units) {
    for (const HlslDecl& decl : a.units[u].scan.decls) addDecl(items, decl);
  }
  if (a.kind == DocumentKind::ShaderLab) {
  ...
  for (const std::string& keyword : hlsl::keywords()) items.add(keyword, kItemKeyword);
  for (const std::string& type : hlsl::types()) items.add(type, kItemClass);
  for (const std::string& function : hlsl::intrinsics()) items.add(function, kItemFunction, "HLSL intrinsic function");
  for (const auto& file : includedFiles(a, units, context)) {
    std::string where = file->path.filename().string();
    for (const HlslDecl& decl : file->scan.decls) addDecl(items, decl, where);
  }
  return items.result();
}
...
const Scope* scopeAt(const Analysis& a, size_t offset) {
  ...
  for (const Scope& scope : a.shader.scopes) {
    ...
    if (contains && (!best || scope.span.begin >= best->span.begin)) best = &scope;
  }
  return best;
}
...
json shaderLabCompletion(const Analysis& a, size_t offset, const FeatureContext& context) {
  ...
  if (!inString) {
    while (!head.empty() && isIdentChar(head.back())) head.pop_back();
  }
  ...
  if (!scope) {
    if (!a.shader.hasShader) {
      items.add("Shader", kItemSnippet, "Shader \"<name>\" { ... }", "Defines a Shader object with a given name.",
                "Shader \"${1:Custom/NewShader}\"\n{\n\t$0\n}", true);
    }
    return items.result();
  }

  auto commandArgs = [&]() {
    if (tokens.empty() || tokens[0].kind != Tok::Ident) return;
    std::string_view command = tokens[0].text;
    if (afterBracket) return addPropertyRefs(items, a, true);
    ...
    if (auto values = ref::commandValues(command); !values.empty() && !iequals(command, "BlendOp")) {
      if (valueCount == 0) items.addValues(values);
    } else if (iequals(command, "BlendOp")) {
      items.addValues(ref::blendOperations());
    } else if (iequals(command, "Blend")) {
      if (valueCount == 0) items.add("Off", kItemValue, "state", "Disables blending.");
      items.addValues(ref::blendFactors());
    } else if (iequals(command, "ColorMask")) {
      if (valueCount == 0) items.addValues(ref::colorMaskValues());
    } else if (iequals(command, "Fallback")) {
      if (valueCount == 0) items.add("Off", kItemValue, "Fallback Off", "Do not use a fallback Shader object.");
      return;
...
std::span<const ref::Entry> tableForCommandArg(const Command& command, bool stencilField) {
  if (stencilField) return ref::stencilFieldValues(command.name);
  if (auto values = ref::commandValues(command.name); !values.empty()) return values;
  if (iequals(command.name, "Blend")) return ref::blendFactors();
  if (iequals(command.name, "ColorMask")) return ref::colorMaskValues();
  return {};
}
...
std::vector<std::pair<const Command*, bool>> allCommands(const ShaderFile& shader) {
  ...
  auto addList = [&](const std::vector<Command>& commands) {
    for (const Command& command : commands) {
      ...
      for (const Command& child : command.children) result.emplace_back(&child, true);
    }
  };
  for (const SubShader& subShader : shader.subShaders) {
    addList(subShader.commands);
    for (const Pass& pass : subShader.passes) addList(pass.commands);
  }
  return result;
}
...
std::vector<std::pair<const std::vector<Tag>*, bool>> allTags(const ShaderFile& shader) {
  ...
  for (const SubShader& subShader : shader.subShaders) {
    ...
    for (const Pass& pass : subShader.passes) result.emplace_back(&pass.tags, true);
  }
  return result;
}
...
json shaderLabHover(const Analysis& a, size_t offset, const FeatureContext& context) {
  ...
  for (const MaterialProperty& property : shader.properties) {
    ...
    if (inside(property.nameSpan, offset)) return hoverResult(a, property.nameSpan, propertyMarkdown(a, property), context);
  }
  for (auto [command, stencilField] : allCommands(shader)) {
    ...
    for (const CommandArg& arg : command->args) {
      if (arg.comma || !inside(arg.span, offset)) continue;
      if (arg.propertyRef) {
      ...
      if (const ref::Entry* entry = ref::find(table, arg.text)) return hoverResult(a, arg.span, valueMarkdown(*entry), context);
      return nullptr;
    }
  }
  ...
  if (const ref::Entry* entry = ref::findKeywordAnywhere(name)) return hoverResult(a, word, entryMarkdown(*entry), context);
  return nullptr;
}
...
const HlslPragma* pragmaAt(const Analysis& a, size_t unit, size_t offset) {
  for (const HlslPragma& pragma : a.units[unit].scan.pragmas) {
    if (inside(pragma.span, offset)) return &pragma;
  }
  return nullptr;
}
...
json hlslHover(const Analysis& a, size_t unit, size_t offset, const FeatureContext& context) {
  ...
  if (const HlslPragma* pragma = pragmaAt(a, unit, offset)) {
    ...
    for (const PragmaArg& arg : pragma->args) {
      if (!inside(arg.span, offset)) continue;
      std::span<const ref::Entry> table;
      if (pragma->name == "target") table = ref::pragmaTargets();
      if (pragma->name == "require") table = ref::pragmaRequires();
      if (pragma->name == "only_renderers" || pragma->name == "exclude_renderers" || pragma->name == "skip_optimizations") table = ref::renderers();
      if (const ref::Entry* entry = ref::find(table, arg.text)) return hoverResult(a, arg.span, valueMarkdown(*entry), context);
      if (!isStagePragma(pragma->name)) return nullptr;
      break;
    }
  }
  for (size_t u : units) {
  ...
  if (word.empty()) return nullptr;
  std::string name = spanText(a, word);
  if (const HlslDecl* decl = findLocalDecl(a, units, name)) return hoverResult(a, word, declMarkdown(*decl), context);
  if (a.kind == DocumentKind::ShaderLab) {
  ...
  if (hlsl::isIntrinsic(name)) return hoverResult(a, word, "```hlsl\n" + name + "\n```\n\nHLSL intrinsic function", context);
  if (hlsl::isKeywordOrType(name)) return nullptr;
...
json symbol(const Analysis& a, std::string name, std::string detail, int kind, Span range, Span selection,
            json children, const FeatureContext& context) {
  if (range.end < selection.end || range.begin > selection.begin) range = selection;
  ...
  if (!detail.empty()) result["detail"] = std::move(detail);
...
int symbolKindFor(DeclKind kind) {
  switch (kind) {
    case DeclKind::Function: return kSymFunction;
    case DeclKind::Struct: return kSymStruct;
    case DeclKind::CBuffer: return kSymNamespace;
    case DeclKind::Variable: return kSymVariable;
    case DeclKind::Field: return kSymField;
    case DeclKind::Macro: return kSymConstant;
  }
  return kSymVariable;
}
...
json declSymbols(const Analysis& a, const HlslScan& scan, const FeatureContext& context) {
  ...
  for (const HlslDecl& decl : scan.decls) {
    if (decl.kind == DeclKind::Field || decl.kind == DeclKind::Macro) continue;
...
json completion(const Analysis& a, size_t offset, const FeatureContext& context) {
  if (auto unit = a.unitAt(offset)) {
    ...
    if (!glsl) return hlslCompletion(a, *unit, offset, context);
    return {{"isIncomplete", false}, {"items", json::array()}};
  }
  if (a.kind == DocumentKind::ShaderLab) return shaderLabCompletion(a, offset, context);
  return {{"isIncomplete", false}, {"items", json::array()}};
}

json hover(const Analysis& a, size_t offset, const FeatureContext& context) {
  if (auto unit = a.unitAt(offset)) return hlslHover(a, *unit, offset, context);
  if (a.kind == DocumentKind::ShaderLab) return shaderLabHover(a, offset, context);
  return nullptr;
}
...
json definition(const Analysis& a, size_t offset, const FeatureContext& context) {
  ...
  if (auto unit = a.unitAt(offset)) {
    ...
    if (word.empty()) return nullptr;
    std::string name = spanText(a, word);
    if (const HlslDecl* decl = findLocalDecl(a, units, name)) return local(decl->nameSpan);
    if (a.kind == DocumentKind::ShaderLab) {
      if (const MaterialProperty* property = a.shader.findProperty(name)) return local(property->nameSpan);
    }
    if (hlsl::isKeywordOrType(name)) return nullptr;
    if (auto external = findExternalDecl(includedFiles(a, units, context), name)) {
  ...
  if (a.kind != DocumentKind::ShaderLab) return nullptr;
  for (auto [command, stencilField] : allCommands(a.shader)) {
    for (const CommandArg& arg : command->args) {
      if (arg.propertyRef && inside(arg.span, offset)) {
        if (const MaterialProperty* property = a.shader.findProperty(arg.text)) return local(property->nameSpan);
        return nullptr;
      }
    }
  }
  for (const MaterialProperty& property : a.shader.properties) {
    if (!inside(property.nameSpan, offset)) continue;
    ...
    for (const HlslUnit& unit : a.units) {
      for (const HlslDecl& decl : unit.scan.decls) {
        if (decl.name == property.name && decl.kind == DeclKind::Variable) return local(decl.nameSpan);
      }
    }
  }
  return nullptr;
...
json documentSymbols(const Analysis& a, const FeatureContext& context) {
  ...
  if (a.kind != DocumentKind::ShaderLab) return declSymbols(a, a.units[0].scan, context);
  ...
  auto blockSymbols = [&](int subShader, int pass) {
    ...
    for (size_t i = 0; i < a.units.size(); ++i) {
      ...
      if (block.subShader != subShader || block.pass != pass) continue;
  ...
  if (!shader.hasShader) return blockSymbols(-1, -1);

  json children = json::array();
  if (!shader.propertiesKeyword.empty()) {
    json properties = json::array();
    for (const MaterialProperty& property : shader.properties) {
      properties.push_back(symbol(a, property.name, property.displayName + " (" + property.type + ")", kSymProperty,
                                  property.span, property.nameSpan, json::array(), context));
    }
  ...
  for (json& block : blockSymbols(-1, -1)) children.push_back(std::move(block));
  for (size_t s = 0; s < shader.subShaders.size(); ++s) {
    ...
    for (size_t p = 0; p < subShader.passes.size(); ++p) {
      ...
      for (const Tag& tag : pass.tags) {
        if (iequals(tag.key, "LightMode")) lightMode = tag.value;
      }
      std::string name = pass.name.empty() ? "Pass" : "Pass \"" + pass.name + "\"";
      subChildren.push_back(symbol(a, name, lightMode, kSymObject, pass.span, pass.keyword,
                                   blockSymbols(static_cast<int>(s), static_cast<int>(p)), context));
    }
    std::string pipeline;
    for (const Tag& tag : subShader.tags) {
      if (iequals(tag.key, "RenderPipeline")) pipeline = tag.value;
    }
    children.push_back(symbol(a, "SubShader", pipeline, kSymModule, subShader.span, subShader.keyword, subChildren, context));
  }
  result.push_back(symbol(a, shader.name.empty() ? "Shader" : shader.name, "Shader", kSymClass, shader.span,
                          shader.nameSpan.empty() ? shader.keyword : shader.nameSpan, children, context));
  return result;
}
