...
void Server::dispatch(const json &message) {
  ...
  if (method == "workspace/didChangeConfiguration") {
    ...
    for (const std::string &uri : uris)
      schedule(uri, std::chrono::milliseconds(0));
    return;
  }
  if (method == "textDocument/formatting") {
    ...
    respond(id, json::array({{{"range", toRange(analysis->text, analysis->lines, {0, analysis->text.size()}, encoding)}, {"newText", std::move(result.text)}}}));
    return;
  }
