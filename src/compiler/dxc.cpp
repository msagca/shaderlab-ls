...
namespace {
  ...
  std::wstring widen(std::string_view text) {
    ...
    for (size_t i = 0; i < text.size();) {
      ...
      int extra = lead < 0x80 ? 0 : lead < 0xE0 ? 1
                                  : lead < 0xF0 ? 2
                                                : 3;
  ...
  std::string normalize(std::string path) {
    ...
    for (size_t at; (at = path.find("/./")) != std::string::npos;)
      path.erase(at, 2);
  ...
  std::set<std::string> includeNames(std::string_view text) {
    ...
    for (size_t at = 0; (at = text.find("include", at)) != std::string_view::npos; at += 7) {
      size_t hash = at;
      while (hash > 0 && (text[hash - 1] == ' ' || text[hash - 1] == '\t'))
        --hash;
      if (hash == 0 || text[hash - 1] != '#')
        continue;
      size_t open = text.find_first_not_of(" \t", at + 7);
      if (open == std::string_view::npos || (text[open] != '"' && text[open] != '<'))
        continue;
  ...
  public:
    IncludeHandler(IDxcUtils *utils, const IncludeOpener &opener, const std::string &mainDir, const fs::path &realDir, std::string_view mainSource)
    ...
    std::map<std::string, fs::path> takeIncludes() {
      return std::move(includes_);
    }
  private:
    struct Folder {
    ...
    HRESULT load(const std::string &requested, IDxcBlob **source) {
      ...
      if (candidates.empty())
        candidates.push_back({&folders_.front(), asked, false}); // an absolute path
      for (const Candidate &candidate : candidates) {
        auto file = opener_(candidate.name, candidate.folder->real);
        if (file && file->text)
          return serve(requested, asked, *file, source);
      }
      return E_FAIL;
    }
    HRESULT serve(const std::string &requested, const std::string &asked, const IncludeFile &file, IDxcBlob **source) {
      ...
      folders_.push_back({slash == std::string::npos ? std::string() : asked.substr(0, slash), file.path.parent_path(), includeNames(*file.text)});
  ...
  std::string loadedFrom(void *library, const fs::path &fallback) {
    ...
    if (GetModuleFileNameW(static_cast<HMODULE>(library), path, MAX_PATH))
      return displayPath(fs::path(path));
...
Dxc::Dxc(const fs::path &library) {
  ...
  if (module)
    create_ = findSymbol(module, "DxcCreateInstance");
  if (create_) {
...
CompileResult Dxc::compile(const std::string &source, const std::string &sourceName, const fs::path &sourceDir, const std::vector<ShaderDefine> &defines, const std::string &entry, const std::string &profile, const IncludeOpener &opener) const {
  return run(source, sourceName, sourceDir, {"-E", entry, "-T", profile}, defines, opener);
}
CompileResult Dxc::preprocess(const std::string &source, const std::string &sourceName, const fs::path &sourceDir, const std::vector<ShaderDefine> &defines, const IncludeOpener &opener) const {
  return run(source, sourceName, sourceDir, {"-P"}, defines, opener);
}
CompileResult Dxc::run(const std::string &source, const std::string &sourceName, const fs::path &sourceDir, std::vector<std::string> arguments, const std::vector<ShaderDefine> &defines, const IncludeOpener &opener) const {
  ...
  if (FAILED(create(CLSID_DxcUtils, IID_PPV_ARGS(&rawUtils))))
    return result;
  ComPtr<IDxcUtils> utils(rawUtils);
  if (FAILED(create(CLSID_DxcCompiler, IID_PPV_ARGS(&rawCompiler))))
    return result;
  ...
  for (const char *flag : {"-HV", "2018", "-Od", "-Vd"})
  ...
  if (FAILED(compiler->Compile(&buffer, pointers.data(), static_cast<UINT32>(pointers.size()), &include, IID_PPV_ARGS(&rawResult)))) {
    return result;
  }
...
namespace {
  ...
  bool splitDiagnostic(std::string_view line, std::string_view &location, std::string_view &severity, std::string_view &text) {
    ...
    for (size_t at = 0; at != std::string_view::npos; at = line.find(": ", at + 1)) {
      ...
      if (at != 0 && !std::isdigit(static_cast<unsigned char>(line[at - 1])))
        continue;
...
std::vector<CompilerMessage> parseDxcMessages(std::string_view output) {
  ...
  for (size_t start = 0; start < output.size();) {
    ...
    if (!splitDiagnostic(line, location, severity, text))
      continue;
