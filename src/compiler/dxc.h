...
class Dxc final : public HlslCompiler {
 public:
  ...
  bool available() const override { return create_ != nullptr; }
  const std::string& loadError() const override { return loadError_; }
  const std::string& libraryPath() const override { return libraryPath_; }
  std::string_view name() const override { return "dxc"; }
  ...
  CompileResult compile(const std::string& source, const std::string& sourceName,
                        const std::filesystem::path& sourceDir, const std::vector<ShaderDefine>& defines,
                        const std::string& entry, const std::string& profile,
                        const IncludeOpener& opener) const override;
  CompileResult preprocess(const std::string& source, const std::string& sourceName,
                           const std::filesystem::path& sourceDir, const std::vector<ShaderDefine>& defines,
                           const IncludeOpener& opener) const override;

 private:
  explicit Dxc(const std::filesystem::path& library);
  CompileResult run(const std::string& source, const std::string& sourceName, const std::filesystem::path& sourceDir,
