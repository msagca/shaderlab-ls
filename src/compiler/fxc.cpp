...
namespace {
  ...
  public:
    IncludeHandler(const IncludeOpener &opener, fs::path mainDir)
      : opener_(opener), mainDir_(std::move(mainDir)) {}
    HRESULT STDMETHODCALLTYPE Open(D3D_INCLUDE_TYPE, LPCSTR fileName, LPCVOID parentData, LPCVOID *data, UINT *bytes) noexcept override {
    ...
    HRESULT STDMETHODCALLTYPE Close(LPCVOID) noexcept override {
      return S_OK;
    }
    ...
    HRESULT open(LPCSTR fileName, LPCVOID parentData, LPCVOID *data, UINT *bytes) {
      ...
      if (parentData) {
        ...
        if (parent != parents_.end())
          includerDir = parent->second.parent_path();
      }
  ...
  struct Macros {
    explicit Macros(const std::vector<ShaderDefine> &defines) {
      for (const ShaderDefine &define : defines)
        list.push_back({define.name.c_str(), define.value.c_str()});
  ...
  for (const std::wstring &candidate : candidates) {
    ...
    if (compile_ && preprocess_) {
      ...
      if (GetModuleFileNameW(module, path, MAX_PATH))
        libraryPath_ = displayPath(fs::path(path));
      return;
    }
...
CompileResult Fxc::compile(const std::string &source, const std::string &sourceName, const fs::path &sourceDir, const std::vector<ShaderDefine> &defines, const std::string &entry, const std::string &profile, const IncludeOpener &opener) const {
  ...
  HRESULT hr = reinterpret_cast<D3DCompileFn>(compile_)(source.data(), source.size(), sourceName.c_str(), macros.list.data(), &include, entry.c_str(), profile.c_str(), flags, 0, &code, &errors);
...
CompileResult Fxc::preprocess(const std::string &source, const std::string &sourceName, const fs::path &sourceDir, const std::vector<ShaderDefine> &defines, const IncludeOpener &opener) const {
  CompileResult result;
  if (!preprocess_)
    return result;
  IncludeHandler include(opener, sourceDir);
  ...
  ID3DBlob *errors = nullptr;
  HRESULT hr = reinterpret_cast<D3DPreprocessFn>(preprocess_)(source.data(), source.size(), sourceName.c_str(), macros.list.data(), &include, &text, &errors);
  result.success = SUCCEEDED(hr);
  result.messages = parseFxcMessages(blobText(errors));
  result.includes = include.takeIncludes();
  if (text)
...
Fxc::Fxc() {
  loadError_ = "FXC (d3dcompiler_47.dll) exists on Windows only.";
}
CompileResult Fxc::compile(const std::string &, const std::string &, const fs::path &, const std::vector<ShaderDefine> &, const std::string &, const std::string &, const IncludeOpener &) const {
  return {};
}
CompileResult Fxc::preprocess(const std::string &, const std::string &, const fs::path &, const std::vector<ShaderDefine> &, const IncludeOpener &) const {
  return {};
}
#endif
...
std::vector<CompilerMessage> parseFxcMessages(std::string_view output) {
  ...
  while (start < output.size()) {
    ...
    for (std::string_view marker : {": error ", ": warning "}) {
      ...
      if (at != std::string_view::npos && (severityAt == std::string_view::npos || at < severityAt))
        severityAt = at;
    }
    if (severityAt != std::string_view::npos) {
      ...
      if (!location.empty() && location.back() == ')') {
        ...
        if (open != std::string_view::npos) {
          ...
          if (*cursor == ',')
            message.column = static_cast<int>(std::strtol(cursor + 1, &cursor, 10));
          if (*cursor == '-')
            message.columnEnd = static_cast<int>(std::strtol(cursor + 1, &cursor, 10));
        }
      }
