...
bool isPunct(const T& token, char c) { return token.kind == TK::Punct && token.text[0] == c; }

class Scanner {
 public:
  Scanner(std::string_view source, Span range) : src_(source), range_(range) {}
 ...
 private:
  ...
  void tokenize() {
    ...
    while (i < end) {
      ...
      if (c == '"' || c == '\'') {
        ...
        while (i < end && isIdentChar(src_[i])) ++i;
        tokens_.push_back({TK::Ident, {begin, i}, src_.substr(begin, i - begin)});
      } else if (std::isdigit(static_cast<unsigned char>(c))) {
        while (i < end && (isIdentChar(src_[i]) || src_[i] == '.')) ++i;
  ...
  size_t directive(size_t hash, size_t end) {
    ...
    while (lineEnd < end) {
      if (src_[lineEnd] == '\n') {
        size_t back = lineEnd;
        while (back > hash && (src_[back - 1] == '\r')) --back;
    ...
    size_t i = hash + 1;
    auto skipSpaces = [&] {
      while (i < lineEnd && (src_[i] == ' ' || src_[i] == '\t')) ++i;
    };
    auto word = [&]() -> Span {
      size_t begin = i;
      while (i < lineEnd && isIdentChar(src_[i])) ++i;
      return {begin, i};
    };
    skipSpaces();
    Span nameSpan = word();
    ...
    if (!whole.empty() && src_[whole.end - 1] == '\r') --whole.end;
    ...
    if (name == "pragma") {
      ...
      pragma.span = whole;
      while (true) {
        while (i < whole.end && std::isspace(static_cast<unsigned char>(src_[i]))) ++i;
        if (i >= whole.end || (src_[i] == '/' && i + 1 < whole.end && src_[i + 1] == '/')) break;
        size_t begin = i;
        while (i < whole.end && !std::isspace(static_cast<unsigned char>(src_[i]))) ++i;
        pragma.args.push_back({std::string(src_.substr(begin, i - begin)), {begin, i}});
      }
      if (!pragma.name.empty()) out_.pragmas.push_back(std::move(pragma));
    } else if (name == "include" || name == "include_with_pragmas") {
      skipSpaces();
      if (i < whole.end && (src_[i] == '"' || src_[i] == '<')) {
        char close = src_[i] == '"' ? '"' : '>';
        size_t begin = ++i;
        while (i < whole.end && src_[i] != close) ++i;
        HlslInclude include;
      ...
      if (!macro.empty()) {
        ...
        if (i < whole.end && src_[i] == '(') {
          size_t k = i + 1;
          while (k < whole.end && (src_[k] == ' ' || src_[k] == '\t')) ++k;
  ...
  enum class Frame { Other, Struct, CBuffer, Function, Initializer };
    ...
    for (size_t k = 0; k < tokens_.size(); ++k) {
      ...
      if (isPunct(token, '{')) {
        ...
        if (depth == 0) {
          ...
          if (frame != Frame::Initializer) statement.clear();
        }
      ...
      if (isPunct(token, '}')) {
        ...
        frames.pop_back();
        if (frames.empty() && frame != Frame::Initializer) statement.clear();
        member.clear();
        continue;
      }
  ...
  Frame classifyBlock(const std::vector<const T*>& statement, const T& brace, std::string& name) {
    if (statement.empty()) return Frame::Other;
    ...
    if (first.kind == TK::Ident && (first.text == "struct" || first.text == "cbuffer" || first.text == "tbuffer") &&
      ...
      addDecl(isStruct ? DeclKind::Struct : DeclKind::CBuffer, *statement[1],
              std::string(first.text) + " " + name, "");
    ...
    for (size_t i = 0; i < statement.size(); ++i) {
      ...
      if (isPunct(token, '[')) ++bracket;
      if (isPunct(token, ']')) --bracket;
      if (bracket == 0 && start == statement.size() && !isPunct(token, ']')) start = i;
      if (isPunct(token, '=') && bracket == 0) return Frame::Initializer;
  ...
  void finishVariable(const std::vector<const T*>& statement, const std::string& container) {
    if (statement.size() < 2) return;
    const T& first = *statement[0];
    if (first.kind == TK::Ident && (first.text == "struct" || first.text == "typedef" || first.text == "return")) return;
    ...
    for (const T* token : statement) {
      ...
      if (isPunct(*token, '(')) return;  // prototype or macro invocation
    }
  ...
  void addVariables(const std::vector<const T*>& statement, DeclKind kind, const std::string& container) {
    ...
    size_t identCount = 0;
    for (const T* token : statement) {
      if (isPunct(*token, '(') || isPunct(*token, '<')) ++nesting;
      if (isPunct(*token, ')') || isPunct(*token, '>')) --nesting;
      if (nesting > 0) continue;
      if (isPunct(*token, ',')) {
        if (candidate && identCount >= 1) addDecl(kind, *candidate, detail, container);
        candidate = nullptr;
        stopped = false;
        continue;
      }
    ...
    if (candidate && identCount >= 2) addDecl(kind, *candidate, detail, container);
  }
...
HlslScan scanHlsl(std::string_view source, Span range) { return Scanner(source, range).run(); }
