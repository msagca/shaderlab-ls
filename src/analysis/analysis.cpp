...
DocumentKind documentKindFor(const fs::path& path) {
  ...
  if (extension == ".shader") return DocumentKind::ShaderLab;
  if (extension == ".compute") return DocumentKind::Compute;
  return DocumentKind::HlslInclude;
}

std::optional<size_t> Analysis::unitAt(size_t offset) const {
  for (size_t i = 0; i < units.size(); ++i) {
    if (units[i].range.containsInclusive(offset)) return i;
  }
  return std::nullopt;
}

bool Analysis::isProgram(size_t unit) const {
  if (kind == DocumentKind::Compute) return true;
  if (kind != DocumentKind::ShaderLab || units[unit].block < 0) return false;
  return isProgramBlock(shader.blocks[units[unit].block].kind);
}

std::vector<size_t> Analysis::visibleUnits(size_t unit) const {
  if (kind != DocumentKind::ShaderLab || units[unit].block < 0) return {unit};
  const CodeBlock& block = shader.blocks[units[unit].block];
  if (block.kind == BlockKind::GlslProgram) return {unit};
  BlockKind includeKind = isCgBlock(block.kind) ? BlockKind::CgInclude : BlockKind::HlslInclude;
  std::vector<size_t> result;
  for (size_t i = 0; i < units.size(); ++i) {
    if (i != unit && shader.blocks[units[i].block].kind == includeKind) result.push_back(i);
  }
  ...
  for (const HlslPragma& pragma : unit.scan.pragmas) {
    ...
    if (isStagePragma(name) && pragma.args.empty()) {
      ...
      if (pragma.args.size() != 1 || !ref::find(ref::pragmaTargets(), pragma.args[0].text)) {
        ...
        addDiagnostic(analysis, span, Severity::Warning,
                      "Unknown #pragma target value. Expected one of: 2.0, 2.5, 3.0, 3.5, 4.0, gl4.1, 4.5, 4.6, 5.0.");
      }
    } else if (name == "require") {
      if (pragma.args.empty()) addDiagnostic(analysis, pragma.nameSpan, Severity::Warning, "#pragma require needs at least one value.");
      for (const PragmaArg& arg : pragma.args) {
      ...
      if (pragma.args.empty()) addDiagnostic(analysis, pragma.nameSpan, Severity::Warning, "#pragma " + name + " needs at least one graphics API.");
      for (const PragmaArg& arg : pragma.args) {
...
void checkProgramEntryPoints(Analysis& analysis, size_t unitIndex) {
  ...
  for (size_t visible : analysis.visibleUnits(unitIndex)) {
    ...
    for (const HlslInclude& include : analysis.units[visible].scan.includes) exempt |= include.withPragmas;
  }
  if (exempt || (vertex && fragment)) return;
  const CodeBlock& block = analysis.shader.blocks[analysis.units[unitIndex].block];
  if (block.pass < 0 && block.subShader >= 0 && !vertex && !fragment) return;  // SubShader-level blocks are for surface shaders
  std::string missing = !vertex && !fragment ? "#pragma vertex and #pragma fragment" : !vertex ? "#pragma vertex" : "#pragma fragment";
...
std::shared_ptr<const Analysis> analyze(fs::path path, std::string text) {
  ...
  if (analysis->kind == DocumentKind::ShaderLab) {
    ...
    for (size_t i = 0; i < analysis->shader.blocks.size(); ++i) {
      ...
      if (block.kind != BlockKind::GlslProgram) unit.scan = scanHlsl(analysis->text, block.content);
  ...
  for (size_t i = 0; i < analysis->units.size(); ++i) {
    if (analysis->kind == DocumentKind::ShaderLab && analysis->shader.blocks[analysis->units[i].block].kind == BlockKind::GlslProgram) continue;
    validatePragmas(*analysis, i);
    if (analysis->kind == DocumentKind::ShaderLab && analysis->isProgram(i)) checkProgramEntryPoints(*analysis, i);
  }
  return analysis;
}
...
void blankEmptyMacroParens(std::string& text, const std::set<std::string>& names) {
  ...
  while (i < text.size()) {
    ...
    if (c == '/' && i + 1 < text.size() && text[i + 1] == '/') {
      ...
      if (!names.contains(text.substr(begin, i - begin))) continue;
      size_t j = i;
...
std::string rewriteForFxc(std::string_view original, bool* pragmaOnce, const std::set<std::string>* zeroParamMacros) {
  ...
  while (start <= hlsl.size()) {
    ...
    if (i < line.size() && line[i] == '#') {
      ...
      if (word == "pragma") {
        ...
        if (name == "once" && pragmaOnce) *pragmaOnce = true;
        if (!ref::isStandardHlslPragma(name)) {
          if (!line.empty() && line.back() == '\r') out.push_back('\r');
          handled = true;
        }
  ...
  if (zeroParamMacros && !zeroParamMacros->empty()) blankEmptyMacroParens(out, *zeroParamMacros);
  return out;
}
...
std::shared_ptr<const CachedFile> loadCachedFile(const fs::path& path) {
  ...
  std::string key = pathKey(path);
  {
    ...
    if (it != cache.end() && it->second.first == time) return it->second.second;
  }
