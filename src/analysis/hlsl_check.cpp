...
int targetValue(std::string_view text) {
  if (text == "2.0") return 20;
  if (text == "2.5") return 25;
  if (text == "3.0") return 30;
  if (text == "3.5") return 35;
  if (text == "4.0") return 40;
  if (text == "4.5") return 45;
  if (text == "4.6" || text == "gl4.1") return 46;
  if (text == "5.0") return 50;
  return 0;
}
...
class Checker {
  ...
  std::vector<Diagnostic> run() {
    ...
    if (a_.kind == DocumentKind::HlslInclude) {
      ...
      for (size_t unit = 0; unit < a_.units.size(); ++unit) {
        if (cancelled_()) break;
        if (a_.isProgram(unit)) checkProgram(unit);
      }
    }
 ...
 private:
  ...
  void collectZeroParamMacros() {
    auto addFrom = [&](const HlslScan& scan) {
      for (const HlslDecl& decl : scan.decls) {
        if (decl.kind == DeclKind::Macro && decl.zeroParams) zeroParamMacros_.insert(decl.name);
      }
    };
    std::vector<std::pair<std::string, fs::path>> queue;
    for (const HlslUnit& unit : a_.units) {
      addFrom(unit.scan);
      for (const HlslInclude& include : unit.scan.includes) queue.emplace_back(include.path, a_.path.parent_path());
    }
  ...
  Span anchorFor(size_t unit) const {
    if (a_.kind == DocumentKind::ShaderLab) return a_.shader.blocks[a_.units[unit].block].keyword;
    return {0, 0};
  }
  ...
  void checkProgram(size_t unit) {
    ...
    for (size_t v : visible) {
      for (const HlslPragma& pragma : a_.units[v].scan.pragmas) pragmas.push_back(&pragma);
    }
    ...
    for (const HlslPragma* pragma : pragmas) {
      ...
      if (name == "only_renderers" && !has(*pragma, {"d3d11", "dx11"})) return;
      if (name == "exclude_renderers" && has(*pragma, {"d3d11", "dx11"})) return;
      if (name == "surface") surface = true;
      ...
      if (name == "require" && has(*pragma, {"compute", "randomwrite", "uav", "tesshw", "tessellation", "msaatex",
                                             "cubearray", "interpolators32", "mrt8", "sparsetex",
      ...
      if (!pragma->args.empty()) {
        ...
        if (!compute) {
          if (name == "vertex") stages.push_back({arg.text, "vs", "SHADER_STAGE_VERTEX", arg.span});
          if (name == "fragment") stages.push_back({arg.text, "ps", "SHADER_STAGE_FRAGMENT", arg.span});
          if (name == "geometry") stages.push_back({arg.text, "gs", "SHADER_STAGE_GEOMETRY", arg.span});
          if (name == "hull") stages.push_back({arg.text, "hs", "SHADER_STAGE_HULL", arg.span});
          if (name == "domain") stages.push_back({arg.text, "ds", "SHADER_STAGE_DOMAIN", arg.span});
        } else if (name == "kernel") {
    ...
    for (const std::string& keyword : dynamicKeywords) source += "uniform bool " + keyword + ";\n";
    for (size_t v : visible) {
  ...
  std::optional<std::string> passDefine(size_t unit) const {
    if (a_.kind != DocumentKind::ShaderLab) return std::nullopt;
    const CodeBlock& block = a_.shader.blocks[a_.units[unit].block];
    if (block.subShader < 0 || block.pass < 0) return std::nullopt;
    for (const Tag& tag : a_.shader.subShaders[block.subShader].passes[block.pass].tags) {
      if (!iequals(tag.key, "LightMode")) continue;
      const ref::Entry* entry = ref::find(ref::tagValues("LightMode"), tag.value);
      if (!entry || entry->detail.find("Built-in") == std::string_view::npos) return std::nullopt;
      std::string define = "UNITY_PASS_";
      for (char c : entry->name) define.push_back(static_cast<char>(std::toupper(static_cast<unsigned char>(c))));
      return define;
    }
    return std::nullopt;
  }
  ...
  void selectKeywords(const HlslPragma& pragma, std::vector<ShaderDefine>& defines, std::vector<std::string>& dynamic) {
    ...
    if (pragma.name.rfind("dynamic_branch", 0) == 0) {
      for (const std::string& k : keywords) {
        if (!isNone(k)) dynamic.push_back(k);
      }
      return;
    }
    bool chosen = false;
    ...
    if (!isNone(keywords[0]) && !singleFeature) defines.push_back({keywords[0], "1"});
  }

  void add(Span span, Severity severity, std::string message, std::string code = {},
           std::optional<RelatedLocation> related = std::nullopt) {
  ...
  Span documentSpan(const CompilerMessage& message) const {
    ...
    if (stop <= begin) {
      stop = begin;
      while (stop < end && isIdentChar(a_.text[stop])) ++stop;
      if (stop == begin && stop < end) ++stop;
    }
    return {begin, stop};
  }
  ...
  bool reaches(const fs::path& file, const std::string& target, std::set<std::string>& visited) const {
    ...
    if (!visited.insert(key).second || visited.size() > 600) return false;
    ...
    for (const HlslInclude& include : cached->scan.includes) {
      ...
      if (resolved && reaches(*resolved, target, visited)) return true;
    }
    return false;
  }
  ...
  void map(const CompilerMessage& message, const CompileResult& result, size_t unit, const Stage* stage) {
    ...
    if (message.warning) return;  // Warnings inside Unity's shader libraries are noise here.

    std::optional<fs::path> resolved;
    if (auto it = result.includes.find(message.file); it != result.includes.end()) resolved = it->second;
    ...
    if (resolved) related = RelatedLocation{displayPath(*resolved), message.line - 1, message.column - 1, message.text};
    add(anchor, severity,
        "In " + where + "(" + std::to_string(message.line) + "," + std::to_string(message.column) + "): " + message.text,
        message.code, related);
  }
