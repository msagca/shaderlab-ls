...
class UnityProject {
 public:
  ...
  const std::filesystem::path& root() const { return root_; }  // empty outside a project
  const std::filesystem::path& editorData() const { return editorData_; }  // empty if not found
  const std::string& editorVersion() const { return editorVersion_; }
