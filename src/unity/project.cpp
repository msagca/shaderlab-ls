...
fs::path findProjectRoot(const fs::path& file) {
  for (fs::path dir = file.parent_path(); !dir.empty(); dir = dir.parent_path()) {
    if (isDirectory(dir / "Assets") && isDirectory(dir / "ProjectSettings")) return dir;
    if (dir == dir.root_path() || dir.parent_path() == dir) break;
  }
  return {};
}
...
std::string readEditorVersion(const fs::path& root) {
  ...
  while (std::getline(lines, line)) {
    ...
    if (view.substr(0, key.size()) == key) return std::string(trim(view.substr(key.size())));
  }
  return {};
}
...
UnityProject::UnityProject(fs::path root, const fs::path& editorOverride) : root_(std::move(root)) {
  editorVersion_ = root_.empty() ? std::string() : readEditorVersion(root_);

  if (!editorOverride.empty()) editorData_ = dataFolderFor(editorOverride);
    ...
    if (editorData_.empty()) {
      ...
      for (const fs::path& hub : roots) {
        ...
        for (const auto& entry : fs::directory_iterator(hub, ec)) {
          if (!isDirectory(entry.path() / "Editor" / "Data")) continue;
          auto numbers = versionNumbers(entry.path().filename().string());
          if (numbers > best) {
            best = numbers;
            editorData_ = entry.path() / "Editor" / "Data";
            if (editorVersion_.empty() || root_.empty()) editorVersion_ = entry.path().filename().string();
          }
        }
      }
    }
  ...
  if (numbers.size() >= 2) versionMacro_ = numbers[0] * 100 + numbers[1] * 10;
    ...
    if (auto text = readFile(root_ / "Packages" / "packages-lock.json")) {
      try {
        ...
        for (const auto& [name, info] : dependencies.items()) {
          if (!info.is_object() || !info.contains("version") || !info["version"].is_string()) continue;
...
std::optional<fs::path> UnityProject::resolvePackage(const std::string& name) const {
  {
    ...
    if (it != packageCache_.end() && isDirectory(it->second)) return it->second;
  }
...
std::optional<fs::path> UnityProject::resolveInclude(std::string_view rawName, const fs::path& includerDir) const {
  ...
  if (asPath.is_absolute()) {
    if (isFile(asPath)) return asPath.lexically_normal();
    return std::nullopt;
  }
  ...
  if (name.rfind("Packages/", 0) == 0) {
    ...
    if (slash != std::string::npos) {
      if (auto package = resolvePackage(name.substr(9, slash - 9))) {
        ...
        if (isFile(candidate)) return candidate.lexically_normal();
      }
    }
  }
  std::vector<fs::path> bases{includerDir};
  if (!root_.empty()) bases.push_back(root_);
  if (fs::path builtin = builtinIncludes(); !builtin.empty()) bases.push_back(builtin);
